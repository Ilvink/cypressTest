...
/******/([
  ...
  /***/ function (module, exports, __webpack_require__) {
     ...
     *
     * @param {String} hash -- fingerprint (thumbprint) сертификата
     * @param {String} dataHash -- хэш данных в HEX строке
     * @param {Boolean} signType -- тип подписи открепленная (true) / присоединенная (false) (default: true)
     * @returns {Promise} -- обещание, которое зарезолвится с данными о подписи {String}
     * */
    function signHash(hash, dataHash, signType) {
      signType = typeof signType === 'undefined' ? true : Boolean(signType);
      return new Promise(function (resolve, reject) {
        getCadesCert(hash).then(function (cert) {
          eval(cryptoCommon.generateAsyncFn(function signHash() {
            var oHashedData = 'yield' + cryptoCommon.createObj('CAdESCOM.HashedData'),
              oSigner = 'yield' + cryptoCommon.createObj('CAdESCOM.CPSigner'),
              sSignedMessage = "",
              store = 'yield' + cryptoCommon.createObj('CAdESCOM.Store');

            'yield' + store.Open(cadesplugin.CAPICOM_CURRENT_USER_STORE, cadesplugin.CAPICOM_MY_STORE, cadesplugin.CAPICOM_STORE_OPEN_MAXIMUM_ALLOWED);
            // Задаем настройки для подписи
            try {
              ...
              var algorithmValue = algorithm != null ? 'yield' + algorithm.Value : null;
              //todo по OID взять правильный алгоритм
              hashAlg = getAlgorithmForHashSign(algorithmValue);

              // Инициализируем объект заранее вычисленным хэш-значением
              // Алгоритм хэширования нужно указать до того, как будет передано хэш-значение
              oHashedData.propset_Algorithm(hashAlg);
              // Передаем данные
              oHashedData.SetHashValue(dataHash);
              ...
              try {
                sSignedMessage = oSignedData.SignHash(oHashedData, oSigner, cryptoConstants.CadesType.CADESCOM_CADES_BES);
              } catch (err) {
                alert("Failed to create signature. Error: " + cryptoConstants.getLastError(err));
                return;
              }
    ...
    var constants = {
      ...
      szOID_CP_RSA: "1.2.840.113549.1.1.1"
    };

    function getAlgorithmForHashSign(certificateAlgorithmOid) {
